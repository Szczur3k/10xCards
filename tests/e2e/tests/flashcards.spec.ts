...
test.describe('Flashcards Management', () => {
  ...
  test('TC-AI-01: AI generation requires authentication', async ({ page }) => {
    ...
    const aiButton = page.locator('[data-testid="ai-generation-btn"], button:has-text("AI"), button:has-text("Generate")').first()
  ...
  test('TC-AI-02: Review interface requires authentication', async ({ page }) => {
    ...
    const reviewButton = page.locator('[data-testid="review-btn"], button:has-text("Review"), button:has-text("Review")').first()
